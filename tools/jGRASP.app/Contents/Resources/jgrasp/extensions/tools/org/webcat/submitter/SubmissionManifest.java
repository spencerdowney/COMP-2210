 ...
 */
  ...
  */
   public void packageContentsIntoStream(OutputStream stream,
         final ILongRunningTask task, IStringEncoder encoder) throws IOException
   {
      task.beginSubtask(3);
      ...
      for (Map.Entry<String, String> entry : parameters.entrySet()) {
         String resolved = resolveParameters(entry.getValue(),
               encoder);
         resolvedParameters.put(entry.getKey(), resolved);
      }
               ...
               throws InvocationTargetException
               {
                  try
                  {
                     if (!assignment.isFileExcluded(item.getFilename())
                        && !task.isCancelled())
                     {
                        packager.addSubmittableItem(item, task);
                     }
                  }
                  catch (Exception e)
                  {
